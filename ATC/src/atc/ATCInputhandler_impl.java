...
/**
 * "Controller" implementation using the same control method as the Classical
 * ATC.
 */
public class ATCInputhandler_impl extends Object implements ATCInputhandler {
  ...
  {
	  char n = command.charAt(0);
	  //boolean pause variable added to keep track of if the game is paused or not.  
      // this is sent through the program following the path (convoluted as it may be) that the start and stop button variables take 
	  boolean pause = false;
	  ...
	  case 'P':
		pause = true;
		atc_obj.pause(pause);//sends the pause variable off to pause the game
		JOptionPane.showMessageDialog( null, "Press ok to resume game" );
		pause = false;
		atc_obj.pause(pause);//sends it off to unpause the game
		break;
	  }
    /*Below is the previous implementation of pressing the buttons, I updated it to the above switch statement. 
